...
/* -------------------------------------------------------------------------- */
/*  Storage adapter — swap this layer for an API client later without touching */
/*  the UI. Every persisted collection goes through `storage`.                 */
/* -------------------------------------------------------------------------- */
...
interface StoreValue {
  ...
  toggleFavorite: (ref: Omit<FavoriteRef, "id" | "addedAt">) => void;
  history: HistoryEntry[];
  pushHistory: (entry: Omit<HistoryEntry, "id" | "visitedAt">) => void;
  clearHistory: () => void;
  searchHistory: string[];
  pushSearch: (q: string) => void;
  clearSearchHistory: () => void;
  promptHistory: PromptHistoryEntry[];
  pushPrompt: (title: string, prompt: string) => void;
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    meta?.setAttribute("content", theme === "dark" ? "#0b0b0a" : "#fbfbf9");
  }, [theme]);
  ...
  const toggleFavorite = useCallback(
    (ref: Omit<FavoriteRef, "id" | "addedAt">) => {
  ...
  const pushHistory = useCallback(
    (entry: Omit<HistoryEntry, "id" | "visitedAt">) => {
